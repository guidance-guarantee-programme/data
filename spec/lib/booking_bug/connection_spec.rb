...
RSpec.describe BookingBug::Connection do
  ...
  describe '#login' do
    ...
    it 'parse the response json' do
      expect(subject.login).to be_a(Hash)
    end

    context 'when authentification is unsuccessful' do
      let(:fake_response) { double(:response, status: 401, body: '{"error":"401 Unauthorized"}') }

      it 'raise an error with the response string' do
        expect { subject.login }.to raise_error(BookingBug::InvalidResponse, '401 Unauthorized')
      end
    end
  end
  ...
  describe '#page' do
    ...
    it 'parse the response json' do
      expect(subject.page('/api/v1.0/admin/1234/bookings', '12345')).to be_a(Hash)
    end
  end
end
